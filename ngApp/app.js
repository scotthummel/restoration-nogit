...
(function (restoration) {
    angular.module('restoration', ['ui.router', 'ngResource', 'ui.bootstrap', 'ui.router.title', 'ngMap', 'ckeditor']).config(function ($stateProvider, $urlRouterProvider, $locationProvider) {
        $stateProvider
            .state('home', {
            url: '/',
            ...
            resolve: {
                post: ['BlogService', '$stateParams', function (BlogService, $stateParams) {
                        return BlogService.slug($stateParams.slug).then(function (res) {
                            if (res === null) {
                                var post = { title: String };
            ...
            resolve: {
                tag: ['TagService', '$stateParams', function (TagService, $stateParams) {
                        return TagService.slug($stateParams.slug).then(function (res) {
                            if (res.length == 0) {
                                var tag = { name: String };
